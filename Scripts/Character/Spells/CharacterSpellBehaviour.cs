...
namespace PV3.Character.Spells
{
    public abstract class CharacterSpellBehaviour : MonobehaviourReference
    {
        ...
        private bool canStatusEffectBeenApplied;
        private bool isHealSpell;
        private int totalSpellValue;

        public void CheckIfSpellComponentIsNullBeforeExecution()
        {
            canStatusEffectBeenApplied = true;
            isHealSpell = false;
            totalSpellValue = 0;
        ...
        {
            // Some Spells attack multiple times, so we need to keep track of the total value of all damage dealt by all attacks.
            canStatusEffectBeenApplied = false;

            for (var i = 0; i < component.numberOfAttacks; i++)
            {
                var damage = CalculateSpellDamage(component);
                totalSpellValue += damage;
        ...
        {
            // Spell dealing a low amount of damage leads to having the Target heal zero health; I rather have the Target heal for a minimum of 1 health.
        ...
        {
            if (!canStatusEffectBeenApplied) return;
